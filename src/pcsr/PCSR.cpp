...
using namespace std;
int doubleNum=0;
long int doubleTime=0;
long int doubleTime_No_Lock = 0;
extern long int record;
...
void PCSR::double_list() {
  doubleNum++;
  struct timeval startD,endD,start1,time1,time2;
  gettimeofday(&startD,NULL);
  const int prev_locks_size = edges.N / edges.logN;
  resizeEdgeArray(edges.N * 2);
  int num_pages=edges.N/512;
  //cout<<"the process count is"<<num_pages/(2*double_granularity)<<endl;
  ...
  edge_t * p0=(edge_t *)aligned_alloc(4096,edges.N*sizeof(*edges.items));
  
  gettimeofday(&time1,NULL);
  void* new_addr=(void*)p0;
  void* old_addr=(void*)edges.items;
  for (int i = 0; i < num_pages/(2*double_granularity);i++) {
    void* p1=mremap(old_addr,4096*double_granularity,4096*2*double_granularity,MREMAP_MAYMOVE | MREMAP_FIXED,new_addr);
    edge_t* old_addr1=(edge_t*)old_addr;
    edge_t* new_addr1=(edge_t*)new_addr;
    old_addr1+=512*double_granularity;
    new_addr1+=1024*double_granularity;
    old_addr=(void*)old_addr1;
    new_addr=(void*)new_addr1;
  }
  //syscall(454,old_addr,new_addr,num_pages,double_granularity);
  edges.items=p0;
   /*for(int i=0;i<edges.N;i++){
    if((i/(double_granularity*512))%2!=0){
      if(edges.items[i].dest!=0 || edges.items[i].value!=0){
        printf("error! %d",i);
        exit(-1);
      }
    }
  ...
  int vk=0,vm=0;
  uint32_t node_indexk;
  uint32_t node_indexm;
  for(int i=0;i<num_pages/(2*double_granularity);i++){
    vk=(2*i+1)*512*double_granularity-1;
    for(;vk>(2*i)*512*double_granularity;vk--){
      if(is_sentinel(edges.items[vk])) {
        node_indexk=edges.items[vk].value;
        if (node_indexk == UINT32_MAX) {
        node_indexk = 0;
        }
        break;
      }
    ...
    if(i==0){
      node_indexm=node_indexk+1;
      continue;
    }
    else if (vk==(2*i)*512*double_granularity){
      continue;
    }else{
      for(int j=node_indexm;j<=node_indexk;j++){
          nodes[j].beginning+=i*512*double_granularity;
          nodes[j-1].end+=i*512*double_granularity;
          if(j==nodes.size()-1){
            nodes[j].end = edges.N - 1;
          }
        }
        node_indexm=node_indexk+1;
    }
  ...
  cout<<"the double granularity is"<<double_granularity<<endl;
  long int doubleThisInterval=1000000*(endD.tv_sec-start1.tv_sec)+(endD.tv_usec-start1.tv_usec);
  doubleTime+=1000000*(endD.tv_sec-startD.tv_sec)+(endD.tv_usec-startD.tv_usec);
  doubleTime_No_Lock+=doubleThisInterval;
  //printf("reallo/total and redis/total= %lf,%lf\n",(double)(1000000*(time1.tv_sec-start1.tv_sec)+(time1.tv_usec-start1.tv_usec))/doubleThisInterval, (double)(1000000*(endD.tv_sec-time1.tv_sec)+(endD.tv_usec-time1.tv_usec))/doubleThisInterval);
  //printf("start time is %ld, end time is %ld\n", 1000000*startD.tv_sec+startD.tv_usec-record, 1000000*endD.tv_sec+endD.tv_usec-record);
  printf("doubletime is %ld, double time (except for lock)is %ld\n", 1000000*(endD.tv_sec-startD.tv_sec)+(endD.tv_usec-startD.tv_usec), doubleThisInterval);
  printf("This is the %dth double operation: comsume the time %ld microseconds.\n\n",doubleNum,1000000*(endD.tv_sec-startD.tv_sec)+(endD.tv_usec-startD.tv_usec));
} 
...
void PCSR::half_list() {
  ...
  for (; j < edges.N; j++) {
    edges.items[j].value = 0;
    edges.items[j].dest = 0;
  }
...
void PCSR::insert(uint32_t index, edge_t elem, uint32_t src, insertion_info_t *info) {
  ...
  if (info != nullptr) {
    ...
    while (density >= density_b.y ) {
      int flagNode=node_index;
      double densityL=0,densityR=0;
      len *= 2;
      if (len <= edges.N) {
        level--;
...
void PCSR::add_edge(uint32_t src, uint32_t dest, uint32_t value) {
  add_edge_parallel(src, dest, value, 0); }
...
PCSR::PCSR(int pcsr_double_granularity,uint32_t init_n, uint32_t src_n, bool lock_search, int domain)
  ...
  double_granularity=pcsr_double_granularity;
  ...
  if (is_numa_available) {
    ...
    edges.items = (edge_t *)aligned_alloc(4096,edges.N * sizeof(*(edges.items)));
  }
...
int PCSR::get_distance(int src, int dest){
  for (int i = nodes[src].beginning + 1; i < nodes[src].end; i++) {
    if (edges.items[i].dest == dest) {
      return edges.items[i].value;
    }
  }
}
...
bool PCSR::got_correct_insertion_index(edge_t ins_edge, uint32_t src, uint32_t index, edge_t elem, int node_index,
  ...
  if (is_null(ins_edge.value)) {
    ...
    if (ind < edges.N) {
      ...
      if (!is_null(item.value) && !is_sentinel(item) && item.dest < elem.dest) {
        return false;
      }
